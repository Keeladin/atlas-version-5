  ...
  return (
    <div className="control-shell">
      ...
      <main className="control-grid">
        ...
        <section className="control-card control-wide"><div className="panel-title">Context</div>{contextStatsError ? <p className="warning-text">{contextStatsError}</p> : contextStats ? <><div className="context-summary-grid"><div><span>Current model input</span><strong>{formatTokens(contextStats.current_context_tokens)}</strong></div><div><span>Full transcript input</span><strong>{formatTokens(contextStats.canonical_transcript_tokens)}</strong></div><div><span>Static seat overhead</span><strong>{formatTokens(contextStats.static_tokens)}</strong></div><div><span>Transcript messages</span><strong>{contextStats.transcript.owner_messages + contextStats.transcript.atlas_messages}</strong></div></div><div className="context-window-table"><div className="context-window-row context-window-head"><span>Window</span><span>Model input</span><span>Conversation</span><span>Messages</span><span>Tools</span></div>{contextStats.windows.map((window) => <div className="context-window-row" key={window.exchanges}><strong>{window.exchanges} exchanges</strong><span>{formatTokens(window.input_tokens)}</span><span>{formatTokens(window.dynamic_tokens)}</span><span>{window.owner_messages} + {window.atlas_messages}</span><span>{window.tool_observations}</span></div>)}</div><p className="control-note">{contextStats.transcript.owner_messages} owner messages · {contextStats.transcript.atlas_messages} Atlas messages · {contextStats.transcript.tool_observations} tool observations · largest message {contextStats.transcript.largest_message_characters.toLocaleString()} characters.</p><p className="control-note">Model input includes Atlas instructions and tool definitions. Conversation is the additional recent-window load above that static seat. The canonical transcript remains untouched.</p></> : <p>Calculating context statistics…</p>}</section>
