...
Atlas Control/runtime remains the single authority source. When the owner approves a managed implementation, Atlas discovers the exact operations it expects to need and records them as `authority_grants` in the immutable task contract. `workspace.tasks.create` defaults to `Ask me`, so the proposal the owner reviews contains the objective, scope, acceptance criteria, checkpoints, and the exact operation grants that will apply. That one approval is the deterministic handoff from chat agreement to managed execution. Task creation preflights those operation IDs against the current capability registry and Control policy before work starts. During that task only, a preflighted grant may promote an `Ask me` operation to `Auto`, so the same approved implementation does not stop later to ask the same permission again. A current Control `Deny`, a disabled capability, an unknown operation, filesystem scope, sandbox rule, or other technical constraint is never bypassed.

This keeps the distinction explicit: Control defines the hard boundary, the owner-approved task contract defines authority inside that boundary for this one workflow, and the runtime enforces both before dispatch. The model cannot add new grants through `task_state_delta`; scope expansion requires a new owner decision or Control change. Managed-task lifecycle operations (`workspace.tasks.create`, `workspace.tasks.cancel`, and `workspace.tasks.resume`) are deliberately non-delegable, so one task cannot create or revive another task to escape its approved authority envelope.
...
Outside an approved managed task, Codex start/send/resume operations default to `Ask me`. Inside a task, only the exact pre-approved operation IDs in `authority_grants` are promoted for that task. One Codex turn may run at a time per coding session. Atlas assigns the session ID before dispatch so task cancellation can identify an in-flight start even before Codex returns its first event. The coding bridge persists cancellation tombstones for not-yet-started sessions, preventing a late launch after the task has already been cancelled. Session state records the Codex thread ID, PID/process group, JSONL event log, result, and timestamps so a later Atlas turn can reconnect rather than invent a new worker. Cancellation fences the task first, then terminates the whole original Codex process group, including surviving child commands; if SIGTERM does not stop the group within the bounded grace period the bridge escalates to SIGKILL and verifies the group is gone before reporting cancellation.
