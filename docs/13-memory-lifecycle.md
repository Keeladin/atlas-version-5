...
The asynchronous reconciliation stage is now implemented by migration `25a12`. It claims candidates with durable lease fencing, evaluates a consistent source/memory snapshot, and semantically chooses `discard`, `retain_short_term`, `create`, `equivalent`, `merge`, or `supersede`. Derived durable-memory mutation uses the shared-state write contract in `23-shared-state-write-contract.md`, so publication succeeds only against the memory revision actually evaluated. Owner lifecycle changes, source-content revision changes, expired/reclaimed leases, and invalidated sources all reject stale publication. Derived writers cannot restore owner-retired/deleted content or merge/supersede owner-directed memory.
