...
Explicit owner remember/correct/retire/restore/delete commands retain their command ledger and serialize through this boundary on `(memory_state, owner)`. They evaluate current state inside the locked mutation callback with `expected_version=null`; successful mutations advance the shared revision. Candidate-driven derived-memory reconciliation now reads that revision in its evaluation snapshot and commits with the matching `expected_version`; a changed owner state produces `version_conflict` and the old semantic proposal is not applied. Candidate lease tokens and canonical transcript `content_revision` provide separate fences for worker ownership and source freshness. Index/capsule/embedding publication uses source-specific compare-and-publish checks rather than pretending every derived index is itself owner memory state.
