...
| Machine-local memory observer snapshots reuse the bounded Control projection, do not read credential/config stores, publish atomically with read-only owner ACLs, and preserve protected auth/control boundaries | `test_memory_observer_snapshot.py`, `test_deployment_permissions.py` | Real PostgreSQL + filesystem/deployment assertions |
