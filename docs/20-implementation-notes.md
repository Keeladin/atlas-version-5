...
## 2026-09-12 — Foreground prompt control hierarchy and provenance audit

A repository-level audit traced every current model call and the model-visible instruction/developer/user/tool text feeding it. The audit found that the foreground conversational instruction had accumulated failure-specific evidence, chronology, memory-lifecycle and runtime-protocol rules until restraint and bookkeeping substantially outweighed positive semantic guidance. The issue was treated as a control-location problem rather than a sampling-temperature problem.

The always-on foreground constitution was reduced from roughly 987 words / 7,095 characters to 334 words / 2,475 characters. Including the hidden runtime-output contract, the instruction reduced from roughly 1,303 words / 9,532 characters to 597 words / 4,526 characters. Detailed memory lifecycle mechanics were removed from ordinary conversation, repeated epistemic cautions were collapsed into proportional verification/source-priority rules, and continuity/memory developer injections were shortened to provenance labels. The hidden task/memory metadata schema remains model-visible but is explicitly separated as a `RUNTIME OUTPUT CONTRACT` rather than conversational policy.

The new foreground hierarchy positively prioritizes owner intent, useful outcomes, semantic judgment, reasoned disagreement, relevant initiative, natural continuity, best-supported commitment and verification proportional to consequence. Runtime-enforced boundaries remain runtime responsibilities. The design and audit provenance are recorded in `24-model-prompt-control-and-audit.md`; the point-in-time audit artifact is `/home/jaco/Workspace/Exports/atlas-v5-model-prompts-audit.txt`, while repository source remains authoritative for later revisions.

Validation after the prompt rebalance and existing mobile conversation-layout fix: **188 backend tests passed** with 217 integration-dependent tests skipped in the local non-database run; **17 frontend tests passed** and the Vite production build completed successfully. No production deployment had occurred at the time of this note.


## 2026-09-13 — Passage-of-Time foreground context (deployed as `9ab22de`)

Atlas now receives a trustworthy owner-local clock and owner-turn timestamps in ordinary foreground context. Canonical transcript timestamps remain UTC-aware at rest; model-visible time is converted through the configured `owner_timezone` (`Africa/Johannesburg`). Only owner turns are stamped, keeping the projection compact while still giving the model one reliable **now** and one reliable **then** from which to infer elapsed time. The scheduled-run path uses the same timezone-aware foreground instruction builder. No schema migration was required.

The semantic rule is deliberately non-deterministic: runtime measures time while the model decides whether elapsed time matters relative to the situation. No fixed gap thresholds or staleness bands were introduced. Time is intended to remain latent context that can change continuity, urgency, freshness and expectations; it should be surfaced explicitly only when that materially improves the response. Stable information must not be treated as changed merely because it is old. See `25-temporal-context.md`.

The Ubuntu host timezone was aligned to `Africa/Johannesburg` so host-local displays match the owner, while RTC/canonical timestamps remain UTC and Chrony/NTP stays below the Atlas reasoning boundary. Release `9ab22de` is deployed and `atlas-v5.service` is active. Local backend validation passed **190 tests with 217 skipped**. GitHub CI run 30 subsequently found one unrelated stale memory-attention wording assertion (**406 passed, 1 failed** backend; frontend passed). Commit `eb2d37b` corrected that stale assertion, and CI run 31 then passed both backend and frontend jobs.

## 2026-09-13 — 25a14: owner notifications, Web Push, and the Desktop Commander connector monitor

The first "system task" for Atlas was to keep the owner's Desktop Commander remote connector usable. The journal of `desktop-commander.service` (uid 1000) reframed the task before any code was written: 51 of 53 restarts since 29 Aug hit `Persisted session invalid: Invalid Refresh Token: Already Used`, after which the connector requests a device code, exits when the code expires after 15 minutes, and is restarted by `Restart=always` with a fresh code, every ~17 minutes, until the owner enters one (75 device-code flows, including a 14 hour run on the night of 9–10 Sep). Restarting the service is therefore the harmful trigger, not a repair. The deliverable became a **read-only monitor plus a generic owner-notification platform**, with the monitor as its first consumer; no `systemctl` authority and no Desktop Commander Remote MCP integration (that is general shell on every enrolled machine and needs its own decision).

Migration `25a14` adds `notifications` (awareness ledger with an outbox column for push), `push_subscriptions` and `host_monitor_state`; its downgrade drops all three, and the rehearsal's expected head moves to `25a14` (a refused chain downgrade rolls back to head because Alembic runs the chain in one transaction). `backend/atlas/notifications/` holds the event model, the severity→channel policy, a synchronous emit/thread core shared by the async service and an ORM `before_flush` hook that mirrors every `OwnerAttentionRow` insert/resolution into the ledger without touching the authority write sites, a VAPID push client, the outbox dispatcher loop, and the `/api/notifications` and `/api/push` routers. The capability family `atlas.notifications` gives the model a redacted `notifications.list` and a rate-limited `notifications.emit` (info/warning only, attributed through a new `current_run_id` context variable), so scheduled runs can push results. Design and the monitor's state table are in `26-notifications.md`.

`backend/atlas/monitors/rdc.py` tails the unit journal with a fixed argv from a stored cursor, keeps only the message text (truncated), cursor and timestamp, matches the known lifecycle lines only, checks liveness from `/proc`, and persists cursor + state + emitted events in one transaction. Journal access is granted by adding `atlas-v5` to `systemd-journal` in `deploy-host.sh`; the unit keeps `ProtectHome=yes`. `python -m atlas.monitors.rdc --replay tests/fixtures/rdc_journal_auth_flow.jsonl` replays this morning's recorded flow (code and device id replaced, timestamps rebased) through the real pipeline under `rdc.replay.*` threads, which is the default host acceptance; a deliberate connector restart is a destructive test of the upstream defect and is opt-in only.

Frontend: the service worker gains `push`/`notificationclick` handlers (thread key as `tag`, quiet re-pushes are silent replacements, `action_required` requires interaction), an Updates section joins Needs You on the main surface and the mobile toggle, and Control gains a Notifications card (support/permission/installed-app state, enable/disable this device, send test, registered devices). Deployment: `bootstrap-push-vapid.sh` provisions the VAPID key as a runtime secret and prints the public key; `ATLAS_PUSH_VAPID_SUBJECT` must be set to a `mailto:` the owner controls before the first deploy with push.

Validation on disposable PostgreSQL 17/pgvector: 441 backend tests pass (the one failing test, `test_pending_attention_renders_conflicts_and_review_backlog_bounded`, also fails on `origin/main` and is unrelated), Ruff clean, 21 frontend tests pass, production frontend build succeeds, and the replay CLI produced the expected `action_required` and `resolved` rows against a database migrated to `25a14`. Not yet deployed at the time of this note; after deploy, enable `atlas.notifications` in Control (new capability rows start disabled) and enable push on the phone from Control.

### Upstream issue draft (Desktop Commander)

**Title:** Remote connector loses its session on every restart: rotated refresh token is not persisted

**Body:** On Linux (`@wonderwhy-er/desktop-commander` 0.2.48, `desktop-commander remote` under a `systemd --user` unit with `Restart=always`), a successful device authorization writes `~/.desktop-commander-device/device.json` once. The connector then refreshes its session in memory while running, but the rotated refresh token is never written back. Any later restart (reboot, unit restart, or the connector exiting after a device code expires) logs `Found persisted session for device …` followed by `Persisted session invalid: Invalid Refresh Token: Already Used`, and starts a new device-authorization flow. Because the connector exits when the 15 minute code expires and systemd restarts it, an unattended machine requests a fresh code every ~17 minutes indefinitely; our journal shows 75 such flows in two weeks, with only 3 `Session restored` across 53 restarts. Expected: persist the rotated refresh token after each refresh (or re-read it before shutdown) so a restart resumes the session. Workaround in use: an external monitor that notifies the owner with the current code.


## 2026-09-13 — 25a15: generic host operations (governed MCP servers, event-driven schedules, unit health monitor)

The connector monitor made Atlas aware; the owner wanted Atlas able to act, without hardcoding every operation (the V3/V4 corner) and without blanket shell. The result is a generic capability described in `27-host-operations.md`. Executor selection followed the owner's order (adapt `ubuntu_mcp_server`, else a stronger existing server, else build): reading the sources showed `ubuntu_mcp_server` has basename allow lists only, no argument policy, no identity model and no maintenance since June 2025, while `openSUSE/systemd-mcp` (polkit-authorized unit control, prebuilt and checksummed) and `tumf/mcp-shell-server` (argv only, hardening, empty child environment, redacted audit) cover the need. Identity switching comes from systemd socket activation with `Accept=yes` (one hardened server process per connection as the new `atlas-tools` user, which cannot read runtime secrets or the owner's home), and argument-level authority for units comes from a polkit rule rendered from `/etc/atlas-v5/config/host-policy.toml`. Desktop Commander Remote is deliberately not part of host control: Atlas's host access must not depend on the thing it may be asked to repair.

Runtime: `CapabilityRuntime.register_executor(..., authority_resolver=)` and `resolve_authority` give per-call authority from arguments (never relaxing a static `forbidden`); `RunExecutor` resolves once per tool call and passes the decision through so the action row and the dispatch agree; `CapabilityFailure` distinguishes a definite failure (`completed`/`before_dispatch` → action `failed`) from an ambiguous one (`uncertain`). `MCPSocketClient` shares the stdio choreography over a unix socket. `backend/atlas/integrations/mcp_servers.py` turns `ATLAS_MCP_SERVERS_FILE` into capability entries and operations, deriving effect and authority from MCP annotations with configuration winning, and reports each server's status to Control. New capability rows start disabled.

Awareness and wake: `backend/atlas/monitors/units.py` watches `ATLAS_HOST_WATCH_UNITS` with `systemctl show` and emits on health changes only; migration `25a15` adds `notifications.wake_claimed_at`; `schedule_kind = "event"` with a JSON filter queues one run per matching notification with the redacted event in the trigger snapshot (`claim_events`), coalescing while a run is outstanding and never waking on the model's own or a task's own notifications. The frontend shows event schedules as "On event".

Deployment: `install-host-mcp.sh` (called from `deploy-host.sh` before the runtime starts) creates `atlas-tools`, installs `systemd-mcp` v0.3.4 (SHA256 verified) and `mcp-shell-server` 1.1.9 (uv tool, pinned), renders the polkit rule and shell environment from the policy, installs `atlas-mcp-systemd` and `atlas-mcp-shell` sockets with their `@.service` templates, and probes both sockets as `atlas-v5`. Proving case: `deployment/host/desktop-commander/` (persistence patch generated from the installed 0.2.48 dist, dry-run verified, `apply.sh` with backups) and `deployment/host/units/` with `install-owner-units.sh`, which moves the owner's units to system units so polkit can govern them and switches the connector monitor to system scope.

Validation on disposable PostgreSQL 17/pgvector: 491 backend tests pass (`test_pending_attention_renders_conflicts_and_review_backlog_bounded` still fails on `origin/main` independently), Ruff clean, 23 frontend tests and the production build pass. Host installation, the polkit rule, the socket probes and the connector patch have not yet been run on ubuntuserver at the time of this note; the acceptance steps are in `27-host-operations.md`.


### Addendum: the owner decides

Review found policy decisions baked into code and scripts: a fixed list of refused shell commands, unit verbs that could never be granted, an installer that refused the docker group, a unit file that hardcoded which server tools existed, model notification severities and event wake sources fixed in code. All of these are now owner configuration with defaults: the host policy carries `identity.groups`, any systemd verb (unit-file verbs are granted for all units because polkit cannot scope them, and the renderer says so), per-action `[[polkit.grants]]` with explicit identities, `systemd.enabled_tools`, and `shell.allow_risky_commands`; the renderer validates shapes and prints a note per risky choice instead of refusing. `ATLAS_NOTIFICATIONS_MODEL_SEVERITIES` sets what the model may emit; event filters accept `include_model`. Migration `25a16` adds `operation_authority`: the Authority card in Control (`/api/control/operations`) lets the owner set every operation to auto, ask or forbidden, and that decision wins over descriptors, server hints, configuration and argument rules alike.

## 2026-09-13 - host operations authority correction

The first governed-host implementation duplicated owner authority across Atlas and an OS-level polkit policy, exposed raw TOML as the primary Control UX, and bootstrapped service-specific grants that were not the intended center of gravity. That shape was corrected before further rollout.

Host authority now has one source of truth: the owner's per-operation settings in Atlas Control (`Automatic`, `Ask me`, `Deny`) plus explicit filesystem path scopes. A local socket-activated root broker performs structured operations only and has no independent policy engine or arbitrary shell. System service, Docker, filesystem, package and host-power actions are separate operation IDs so owner choices map to real dispatch boundaries rather than cosmetic UI groupings. The old systemd/shell MCP sockets and host-policy path are retired by the installer and the generated Atlas polkit rule is removed. The main Atlas runtime remains unprivileged.

The installer migrates only Atlas host MCP blocks and preserves unrelated/custom MCP server definitions. `docs/27-host-operations.md` is the canonical description of the corrected design.

## 2026-09-13 — Repository browser: local/remote commit and CI state

The Repositories view now expands each GitHub repository in place instead of opening GitHub immediately. Detail is loaded lazily so the compact list stays fast and vertical growth is owner-controlled. The expanded view shows the live remote default-branch commit, GitHub check/status summary, every matching local checkout/worktree under the read-only Projects mount, branch/SHA/subject, clean vs dirty worktree state, and ahead/behind/diverged state against the local origin tracking ref.

Linked worktrees need special handling because their `.git` file contains an absolute owner path such as `/home/jaco/Projects/.../.git/worktrees/...`, while the runtime sees Projects at `/var/lib/atlas-v5/projects`. Repository status remaps that gitdir into the runtime mount for read-only Git inspection and projects the displayed path back to the owner's `/home/jaco/Projects` namespace. Remote commit and CI state use the already-protected GitHub token server-side; credentials are never returned to the browser.


## 2026-09-14 — Google Workspace OAuth, Gmail completion and action-budget cleanup

Google Workspace was reauthorized against the Control-managed OAuth bundle at `/var/lib/atlas-v5/control/google-workspace-config`. The OAuth client file was recovered and the managed bundle now self-heals a missing `client_secret.json` from the stored authorized-user client metadata. Interactive authorization explicitly requests `https://mail.google.com/`, Drive, Calendar and identity scopes; relying on `--services drive,gmail,calendar` is no longer sufficient because the Gmail service selector yields the narrower mutation scope and does not authorize permanent delete. The helper is documented in `28-google-workspace-oauth.md`.

Runtime credential precedence was corrected so an encrypted `credentials.enc` token takes priority over the legacy `authorized_user.json` import/fallback. This closed the mismatch where manual `gws auth status` showed the new full Gmail scope while Atlas commands were still forced onto an older token. Gmail acceptance testing now covers search/read, trash, send, threaded reply and permanent delete; the permanent-delete test was independently verified by reading the exact deleted message ID and receiving Gmail's not-found response.

The capability completion reserve also had two runtime defects exposed by this test. Capability discovery results were not being copied into the per-turn budget metadata, so effecting operations could be misclassified and blocked by the reserve intended to protect them. In addition, resolved/acknowledged action references could persist in active-task checkpoints across owner turns, forcing repeated `evidence.task.read` paging and consuming most of the 16-call operational budget. Discovery now teaches the budget operation effects, and checkpoint reconciliation deterministically prunes canonically terminal or owner-resolved actions outside the model dispatch budget while retaining genuinely unresolved effects fail-closed.

Commits for the repair sequence are `ebea0df` (repair incomplete managed OAuth bundle), `b30be0e` (prefer current encrypted credentials), `056f5b2` (teach capability budget from discovery), and `5bb17a0` (prune resolved action backlog). Full backend validation after the checkpoint cleanup: **293 passed, 235 skipped**, Ruff clean.

## 2026-09-15 — Chat deletion no longer cascades through memory provenance

Production logs exposed two failures when deleting owner chats that had participated in the memory state machine: deleting a source candidate could leave a `memory_provenance` row with no live source and violate `ck_memory_provenance_has_source`, while deleting an owner assertion turn could violate the durable-memory `RESTRICT` grounding foreign key. Both were symptoms of the same lifecycle mismatch: raw conversation is temporary, but verified durable memory and its provenance may outlive that conversation.

Owner-chat deletion now tombstones the source identities instead of physically cascading them. The chat leaves all owner/continuity/discovery surfaces, its title/summary/task state and exact turn payloads are scrubbed, its searchable chunks/capsules/discovery state are removed, and its content revision advances to fence in-flight publishers. Durable memories and provenance links remain structurally valid. Candidates sourced from the deleted chat, including blocked candidates, are invalidated and have their semantic payload scrubbed while their IDs remain as lineage tombstones. The regression reproduces both the verified owner-assertion link and a blocked candidate used by a provenance row. Validation against disposable PostgreSQL 17: **528 backend tests passed**, with Ruff clean.


## 2026-09-15 — First-class provider image generation and editing

Atlas already had the inbound half of multimodal work: owner attachments are frozen as artifacts/evidence and image resources are projected to the conversational OpenAI model as native `input_image` content. This change leaves that path intact and adds the missing outbound boundary instead of creating a second image-ingestion subsystem.

The new provider capability `openai.images` exposes `image.generate` and `image.edit`. Both are `create` effects with an `Automatic` default; the owner's operation-authority override still wins. Newly provisioned capability rows remain disabled by default in the persistent Environment Registry, so the owner must enable Image generation in Control before Atlas can discover or dispatch it. The provider model is configurable with `ATLAS_OPENAI_IMAGE_MODEL` and defaults to `gpt-image-2.5-sunburst`.

`OpenAIImageService` invokes the provider, decodes returned image bytes only transiently, persists successful output through the existing Artifact Store, and returns artifact identity/metadata rather than image bytes to the capability/evidence path. `image.edit` consumes the exact source `artifact_id`; missing, non-image, or unavailable source content fails before provider dispatch. Known provider failures are terminal failed actions rather than unresolved/uncertain effects.

Successful image artifacts are collected by `RunExecutor`, attached as `artifact_ref` blocks to the final Atlas turn, served through the authenticated `/api/artifacts/{artifact_id}` route, and rendered inline by the existing conversation UI with a full-image/Save image affordance. The artifact store now resolves read/delete paths beneath its configured root before access.
...
## 2026-09-15 — Durable foreground run observation over SSE

Foreground conversation transport now separates owner command submission from run observation. `POST /api/conversation/runs` durably appends the owner turn, creates the fenced foreground run and its first run event, then returns `202 Accepted`; a runtime-owned background task performs inference independently of the HTTP observer. `GET /api/runs/{run_id}/events` exposes a same-origin authenticated SSE projection with event IDs, `Last-Event-ID` replay, 10-second keep-alives and no proxy buffering. Conversation snapshots expose an active foreground run so a refreshed browser can reattach automatically instead of creating a second turn.

Migration `25a17` adds the durable `run_events` ledger. Accepted, running, delta, completed and interrupted events are server-owned; interruption records are emitted by the runtime recovery path rather than inferred by the browser. Native `EventSource` reconnect errors are intentionally non-terminal. The existing foreground uniqueness fence, task checkpoint, action ledger and uncertain-effect rules remain unchanged.

Validation on a disposable PostgreSQL 17/pgvector instance: clean migration through `25a17`, `alembic check` with no drift, **540 backend tests passed** on the implementation tree, and the final focused durable-run/SSE suite passed **4/4** including active-run discovery and cursor replay. Ruff and `git diff --check` pass. Frontend lint reports **0 warnings / 0 errors**, **36 frontend tests pass**, and the Vite production build succeeds. CI will provide the final consolidated-commit gate after merge to `main`.
