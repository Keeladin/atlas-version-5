...
The discovery sweep had never accepted a candidate because it minted evidence handles from ORM rows; it now converts to domain turns, checks for live unscanned turns, advances by compare-and-set, rotates failed/stale transcripts, and is rewound by purges. Memory conflicts gained an owner resolution path (`keep_current`, `accept_competing`, `restate`) under a review-version CAS, a projection that exposes both wordings, foreground attention injection, and a clarification round trip exercised through the capability runtime.
