...
Implementation checkpoint (2026-09-09): the model-led conversation path now supports separately named owner chats with create/switch/rename/delete controls and per-chat canonical transcripts; progressive capability runtime, generic resource acquisition, local workspace, Google Drive/Gmail, read-only GitHub MCP, provider-native web search, approval widgets, provider-token context-pressure telemetry, workspace/project operations, schedules and action reconciliation are implemented. Memory now includes deterministic transcript chunking/checkpointing, bounded hybrid PostgreSQL full-text/pgvector recall with lexical fallback, evidence-grounded historical verification, explicit owner-directed remember/correct/retire/restore/delete commands with precedence guards, cross-chat continuity capsules, bounded foreground **memory-candidate hint intake**, and a bounded asynchronous candidate reconciler that publishes only derived memory under lease/source/memory revision fences. `equivalent` provenance, short-term retention/review, and source-fenced index/capsule/embedding publication are implemented. Explicit remember/correct are routed through the same verification pipeline, a bounded incremental discovery sweep is implemented, and memory conflicts are owner-resolvable. Whole-history consolidation, visual attachment indexing and OEM-manual ingestion remain staged. See `20-implementation-notes.md` for implementation evidence and recovery remediation.
...
Explicit `remember`, `correct`, `retire`, `restore`, and `delete` instructions receive durable command state and precedence over stale derived memory.
