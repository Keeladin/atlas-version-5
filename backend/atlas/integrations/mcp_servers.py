"""Owner-configured MCP servers as Atlas capabilities.

MCP describes capability and effect only. Owner authority is resolved centrally by
Atlas Control through operation_authority; an MCP server is never a second policy
engine and never decides whether Atlas may invoke a tool.
"""
...
RESERVED_PREFIXES = (
    "atlas", "openai", "github", "google", "evidence", "memory",
    "storage", "schedules", "notifications",
)


@dataclass(frozen=True)
class ToolPolicy:
    """Capability metadata only; authority belongs to Atlas Control."""
...
def _effect(value: Any, *, where: str) -> EffectKind | None:
    ...
    try:
        ...
        raise ValueError(
            f"{where}: effect must be read, create, update, delete or execute"
        ) from exc


def _tool_policy(name: str, raw: dict[str, Any], *, where: str) -> ToolPolicy:
    # Older files may still contain authority/authority_rules.  They are
    # intentionally ignored: Control / operation_authority is the single
    # authority source.
...
def parse_mcp_servers(text: str) -> list[MCPServerConfig]:
    ...
    for index, raw in enumerate(servers):
        ...
        if server_id.split(".", 1)[0] in RESERVED_PREFIXES:
            prefix = server_id.split(".", 1)[0]
            raise ValueError(f"{where}: id prefix '{prefix}' is reserved for built-in capabilities")
        ...
        if tools_raw is not None:
            if not isinstance(tools_raw, list) or not all(
                isinstance(item, str) and TOOL_NAME.match(item) for item in tools_raw
            ):
        ...
        policies = {
            str(name): _tool_policy(str(name), policy, where=f"{where}.tool")
        ...
        result.append(MCPServerConfig(
            id=server_id,
            family=str(raw.get("family") or server_id),
            description=str(raw.get("description") or f"MCP server {server_id}."),
            transport=transport,
            path=path,
            command=command,
            args=tuple(args),
            env_file=Path(str(raw["env_file"])) if raw.get("env_file") else None,
            trust=trust,
            tools=tools,
...
def descriptor_for_tool(server: MCPServerConfig, tool: dict[str, Any]) -> OperationDescriptor | None:
    ...
    return OperationDescriptor(
        ...
        input_schema=schema,
        effect=effect,
        # This is only the descriptor fallback. Owner overrides in the central
        # operation_authority store are the policy source of truth.
...
class GovernedMCPServer:
    """Transport adapter. Governance happens in Atlas, before call()."""
    ...
    def __init__(self, config: MCPServerConfig, client: MCPClientBase | None = None) -> None:
        ...
        if client is not None:
            ...
            self.client = MCPStdioClient(
                config.command,  # type: ignore[arg-type]
                list(config.args),
                environment=_read_env_file(config.env_file),
    ...
    def call(self, operation_id: str, arguments: dict[str, Any]) -> Any:
        ...
        try:
            ...
            raise CapabilityFailure(
                str(exc), phase="completed", output={"tool": name, "error": str(exc)}
            ) from exc
        except MCPUnavailableError as exc:
            raise CapabilityFailure(
                str(exc), phase="before_dispatch", output={"tool": name}
            ) from exc
        except MCPTransportError as exc:
            raise RuntimeError(str(exc)) from exc

    def authority_resolver(self, descriptor: OperationDescriptor):
        # Kept for the runtime registration interface. MCP has no authority
        # resolver; OperationAuthorityRepository is the central policy layer.
        return None


def register_mcp_servers(settings: Settings, registry, runtime) -> list[dict[str, Any]]:
    """Add every configured server as a capability; discovery failures remain visible."""
    try:
        ...
        return [{
            ...
            "operations": [],
            "transport": str(settings.mcp_servers_file),
            "error": str(exc),
        }]
    ...
    for config in configs:
        ...
        entry = CapabilityEntry(
            ...
            trust=config.trust,  # type: ignore[arg-type]
        )
        ...
        if configured:
            ...
            for descriptor in descriptors:
                ...
                runtime.register(
                    descriptor,
                    lambda arguments, operation_id=descriptor.id, server=server:
                        server.call(operation_id, arguments),
