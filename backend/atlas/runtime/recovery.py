...
async def interrupt_run(factory, artifacts, run_id, *, reason: str) -> None:
    async with factory() as session:
        ...
        # uncertain. Never replay it merely because an inference run stopped.
        actions = (
            await session.execute(
                select(ActionRow).where(
                    ActionRow.run_id == run_id,
                    ActionRow.status == "executing",
                )
            )
        ).scalars().all()
        for action in actions:
            changed = await session.execute(
                update(ActionRow)
                .where(ActionRow.id == action.id, ActionRow.status == "executing")
            ...
            if changed.rowcount == 1:
                ...
                if run.transcript_id is not None:
                    await EvidenceStore(session, artifacts).record(
                        ...
                        phase="uncertain",
                        detail={"status": "uncertain", "reason": reason},
                        run_id=run_id,
                        action_id=action.id,
                        checkpoint=run.kind == "foreground",
                        trust="internal",
                    )
        ...
        if not managed_task:
            existing = (
                await session.execute(
                    select(OwnerAttentionRow.id).where(
                        OwnerAttentionRow.run_id == run_id,
                        OwnerAttentionRow.action_id.is_(None),
                        OwnerAttentionRow.state == "interrupted",
            ...
            if existing is None:
                session.add(
                    OwnerAttentionRow(
                        run_id=run_id,
                        state="interrupted",
                        title="Atlas work was interrupted",
                        detail={
                            "message": reason,
...
async def recover_abandoned_runs(factory, artifacts, *, stale_seconds: int = 300) -> int:
        ...
        ids = (
            await session.execute(
                select(RunRow.id).where(
                    RunRow.inference_status == "running",
                    or_(RunRow.heartbeat_at < cutoff, RunRow.heartbeat_at.is_(None)),
                )
            )
        ).scalars().all()
    recovered = 0
    for run_id in ids:
        async with factory() as session:
            ...
            if (
                run is None
                or run.inference_status != "running"
                or (run.heartbeat_at and run.heartbeat_at >= cutoff)
            ):
                continue
            run.inference_active = False
            await session.commit()
        await interrupt_run(
            ...
            artifacts,
            run_id,
            reason="Inference stopped before a durable completion. Task state and effect evidence were retained.",
        )
        recovered += 1
    return recovered
