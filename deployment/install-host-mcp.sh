#!/usr/bin/env bash
# Install Atlas V5 structured privileged host operations.
# Owner authority is stored and enforced by Atlas Control. This broker exposes structured
# operations only; it has no independent policy engine and no arbitrary shell tool.
set -euo pipefail
...
if [[ ! -f ${MCP_SERVERS} ]]; then
  ...
  # Replace only Atlas's host server blocks; preserve every unrelated/custom MCP server.
  python3 - "${MCP_SERVERS}" "${ROOT_DIR}/deployment/mcp-servers.example.toml" <<'PY_MCP'
import re, sys
from pathlib import Path
current, host = map(Path, sys.argv[1:])
text = current.read_text()
parts = re.split(r'(?m)(?=^\[\[servers\]\]\s*$)', text)
kept = []
for part in parts:
    match = re.search(r'(?m)^id\s*=\s*"([^"]+)"\s*$', part)
    if match and match.group(1) in {"host.systemd", "host.shell", "host.operations"}:
        continue
    if part.strip():
        kept.append(part.rstrip())
merged = "\n\n".join(kept + [host.read_text().rstrip()]) + "\n"
...
for unit in atlas-host-operations.socket atlas-host-operations@.service; do
...
echo "Atlas structured host operations are installed. Authority is controlled in Atlas Control; polkit is not part of this execution path."
