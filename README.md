...
- **Durable conversation:** PostgreSQL stores owner, Atlas, and structured tool-evidence turns. At 70% context pressure, older history becomes a durable semantic capsule while the latest 40 turns remain verbatim; the canonical transcript is not deleted or truncated.
...
- **Project folders:** Atlas can inspect repositories and perform bounded single-file create/update, preview unified diffs, detect stale files with SHA-256, write atomically, preserve file modes, checkpoint dirty Git state, and move files within a project. Deletes require approval. Protected paths and secret/key material are excluded from normal project writes.
- **Interface:** React and TypeScript provide the chat/control interface, activity and approval surfaces, responsive mobile layout, Markdown rendering, attachments, and an installable PWA.
