...
- **Evidence-grounded hybrid transcript recall:** restart-safe, provenance-backed text chunks are derived from canonical transcript history, indexed with PostgreSQL full-text search, and embedded into pgvector for semantic similarity. `memory.search` combines lexical and semantic candidates with bounded hybrid ranking, reports structural coverage/indexing gaps and safe sequence-boundary exclusions, retains exact provenance, supports iterative exclusions, and falls back to lexical recall if query embedding is temporarily unavailable. Exact evidence reads identify whether the source is an owner statement, prior Atlas/model statement, owner attachment, or tool observation so the model can distinguish relevance from proof. The active conversation tail remains excluded from maintenance indexing. Context-capsule generation, durable remember/correct/forget semantics, semantic memory promotion/reconciliation, visual attachment indexing, and OEM-manual ingestion remain staged work.
...
Atlas does not currently provide general shell or code execution. GitHub remains read-only, project changes are staged for owner integration, and consequential external effects remain owner-gated. Transcript hybrid lexical/semantic recall is now implemented on PostgreSQL full-text search plus pgvector; durable personal-memory curation, visual attachment indexing, and indexed OEM-manual retrieval remain staged work.
...
**Status:** Phase 0 foundation complete; Phase 1 working Atlas centre operational; Phase 5 memory retrieval now provides deterministic transcript indexing, hybrid lexical/semantic recall, and evidence-grounded historical verification signals, while durable remember/correct/forget semantics, memory reasoning, capsules, and visual attachment indexing remain staged.
