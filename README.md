...
- **Indexed transcript recall:** the first memory-orbit slice derives restart-safe, provenance-backed text chunks from canonical transcript history and indexes them with PostgreSQL full-text search. The active conversation tail is deliberately excluded from maintenance indexing, indexing never runs on the foreground inference path, and `memory.search` supports bounded lookup plus per-search exclusions for iterative refinement. Embeddings, context-capsule generation, semantic memory promotion/reconciliation, and OEM-manual ingestion remain staged work.
...
Atlas does not currently provide general shell or code execution. GitHub remains read-only, project changes are staged for owner integration, and consequential external effects remain owner-gated. Transcript lexical recall is now implemented as the first memory slice; durable personal-memory curation, semantic/vector retrieval, and indexed OEM-manual retrieval remain staged work.
...
**Status:** Phase 0 foundation complete; Phase 1 working Atlas centre operational; the first Phase 5 memory slice now provides deterministic transcript indexing and lexical recall while embeddings and durable memory reasoning remain staged.
