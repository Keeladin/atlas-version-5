...
- **Hybrid transcript recall:** restart-safe, provenance-backed text chunks are derived from canonical transcript history, indexed with PostgreSQL full-text search, and embedded into pgvector for semantic similarity. `memory.search` combines lexical and semantic candidates with bounded hybrid ranking, retains exact provenance, supports per-search exclusions, and falls back to lexical recall if query embedding is temporarily unavailable. The active conversation tail remains excluded from maintenance indexing. Context-capsule generation, durable semantic memory promotion/reconciliation, visual attachment indexing, and OEM-manual ingestion remain staged work.
...
**Status:** Phase 0 foundation complete; Phase 1 working Atlas centre operational; Phase 5 memory retrieval now provides deterministic transcript indexing plus hybrid lexical/semantic recall, while durable memory reasoning, capsules, and visual attachment indexing remain staged.
