...
- **Deployment:** FastAPI, PostgreSQL, Alembic, systemd, and Caddy. Production runs as the unprivileged `atlas-v5` user on `127.0.0.1:8086`, with secrets held outside the application tree. Atlas pins its application interpreter in `.python-version` and deploys that exact uv-managed Python independently of Ubuntu's system Python.
